...
class VGG16(Model):
    ...
    def __init__(self, pretrained=False):
        ...
        if pretrained:
            self.load_weight(utils.get_file(VGG16.WEIGHTS_PATH))
    ...
    def preprocess(image, size=(224, 224), dtype=np.float32):
        ...
        # 1. Convert to 224 * 224 ndarray
        if size:
            image = image.resize(size)
        image = np.asarray(image, dtype=dtype)
        image = image[:, :, ::-1]
        # 2. Substract average value of each channel to every pixel
        image -= np.array([103.939, 116.779, 123.68], dtype=dtype)
        # 3. Reorder the channels : B-G-R
